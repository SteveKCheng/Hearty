...
public class MockWorkerHosts
{
    ...
    /// </summary>
    /// <param name="url">
    /// The URL to the WebSocket endpoint to receive the workers.
    /// </param>
    /// <param name="count">
    ...
    /// </param>
    public async Task CreateFakeWorkersAsync(Uri url,
                                             int count,
                                             int concurrency,
                                             string namePrefix,
                                             Func<string, int, IJobSubmission> factory)
    {
        if (!url.IsAbsoluteUri || !IsWebSocketsUrl(url))
            throw new ArgumentException("Connection URL must be for the WebSockets protocol. ", nameof(url));

        _logger.LogInformation("Fake workers will connect by WebSockets on URL: {url}", url);
